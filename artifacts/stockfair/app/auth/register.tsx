...
import {
  ...
  ScrollView, KeyboardAvoidingView, Platform, ActivityIndicator, Animated as RNAnimated,
} from 'react-native';
...
import { useAuth, StokvelTypePreference } from '@/context/AuthContext';
...
const STEPS = ['Personal', 'Contact', 'Language', 'Preferences'];
      ...
      <View style={hdr.topRow}>
        ...
        <View style={hdr.dotsRow}>
          {Array.from({ length: total }, (_, i) => (
            <View key={i} style={[hdr.dot, i < step && hdr.dotDone, i === step && hdr.dotActive]} />
          ))}
        </View>
        <Text style={hdr.stepCount}>{step + 1} of {total}</Text>
      </View>
  ...
  return (
    <View style={sc.card}>
      <View style={sc.nameRow}>
        <View style={{ flex: 1 }}>
          <InputField label="First Name" value={data.firstName} onChangeText={(v: string) => { set('firstName', v); clearErr('firstName'); }}
            placeholder="e.g. Thandi" icon="user" error={errors.firstName} returnKeyType="next" onSubmitEditing={() => lastRef.current?.focus()} />
        </View>
        <View style={{ flex: 1 }}>
          <InputField label="Last Name" value={data.lastName} onChangeText={(v: string) => { set('lastName', v); clearErr('lastName'); }}
            placeholder="e.g. Dlamini" icon="user" error={errors.lastName} inputRef={lastRef} returnKeyType="done" />
        </View>
      </View>

      <InputField label="Date of Birth" value={data.dob} onChangeText={(v: string) => { set('dob', v); clearErr('dob'); }}
        placeholder="DD/MM/YYYY" icon="calendar" keyboardType="numbers-and-punctuation" autoCapitalize="none"
        error={errors.dob} hint="As it appears on your official ID document" />
...
function Step2Contact({ data, set, errors, clearErr }: any) {
  ...
  return (
    <View style={sc.card}>
      <InputField label="Email Address" value={data.email} onChangeText={(v: string) => { set('email', v); clearErr('email'); }}
        placeholder="you@example.com" icon="mail" keyboardType="email-address" autoCapitalize="none" error={errors.email}
        returnKeyType="next" onSubmitEditing={() => phoneRef.current?.focus()} />

      <InputField label="SA Mobile Number" value={data.phone} onChangeText={(v: string) => { set('phone', v); clearErr('phone'); }}
        placeholder="+27 82 555 0123" icon="phone" keyboardType="phone-pad" autoCapitalize="none" error={errors.phone}
        hint="Used for group contributions and OTP verification" inputRef={phoneRef} returnKeyType="next" onSubmitEditing={() => passRef.current?.focus()} />

      <InputField label="Password" value={data.password} onChangeText={(v: string) => { set('password', v); clearErr('password'); }}
        placeholder="Min. 8 characters" icon="lock" secureTextEntry={!showPass} error={errors.password}
        inputRef={passRef} returnKeyType="next" onSubmitEditing={() => confirmRef.current?.focus()}
      ...
      <InputField label="Confirm Password" value={data.confirm} onChangeText={(v: string) => { set('confirm', v); clearErr('confirm'); }}
        placeholder="Re-enter your password" icon="check-circle" secureTextEntry={!showConf} error={errors.confirm}
  ...
  return (
    <View style={sc.card}>
      <Text style={sc.sectionNote}>
        StockFair supports all 11 official languages. Choose your preferred language for the app experience.
      </Text>
...
function Step4Preferences({ data, set, errors, clearErr, agreed, setAgreed, agreeError }: any) {
  ...
  return (
    <View style={sc.card}>
      <Text style={sc.sectionNote}>
        Select the stokvel types you're interested in. You can join or create any type later.
      </Text>
      ...
      <TouchableOpacity style={sc.agreeRow} onPress={() => setAgreed(!agreed)} activeOpacity={0.8}>
        ...
        <Text style={sc.agreeText}>
          I agree to StockFair's{' '}
          <Text style={sc.agreeLink}>Terms of Service</Text>,{' '}
...
const sc = StyleSheet.create({
  ...
  stokvelDesc:     { fontSize: 11, color: '#9E9E9E', flex: 1 },
...
export default function RegisterScreen() {
  ...
  const validateStep = (): boolean => {
    ...
    if (step === 1) {
      ...
      else if (!/^(\+27|0)[6-8][0-9]{8}$/.test(data.phone.replace(/\s/g, ''))) e.phone = 'Enter a valid SA mobile number.';
  ...
  const handleSubmit = async () => {
    if (!agreed) { setAgreeError('You must accept the Terms & Conditions.'); Haptics.notificationAsync(Haptics.NotificationFeedbackType.Error); return; }
    ...
    const result = await register({
      ...
      language: data.language, stokvelPreferences: data.stokvelPreferences,
    });
    setLoading(false);
    if (result.ok) {
  ...
  return (
    <KeyboardAvoidingView style={{ flex: 1, backgroundColor: '#F5F5F5' }} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
      ...
      {/* Back button (outside scroll so it's always accessible) */}
      ...
      <ScrollView contentContainerStyle={{ flexGrow: 1 }} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
        ...
        {/* Header */}
        <StepHeader step={step} total={STEPS.length} title={meta.title} sub={meta.sub} insets={insets} />
          ...
          {isLast ? (
            <TouchableOpacity style={top.submitBtn} onPress={handleSubmit} disabled={loading} activeOpacity={0.87}>
              {loading ? (
                ...
                <>
                  <Icon name="shield" size={17} color="#FFFFFF" />
                  <Text style={top.submitTxt}>Create My Account</Text>
                </>
              )}
            </TouchableOpacity>
          ) : (
            <TouchableOpacity style={top.nextBtn} onPress={handleNext} activeOpacity={0.87}>
              <Text style={top.nextTxt}>Continue</Text>
